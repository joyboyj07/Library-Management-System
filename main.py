...
class main:
    def __init__(self) -> None:
        ...
        self.root = ck.CTk()
        self.root.title("Library Management System (VCET, Viraganoor)")
    ...
    def delete_book(self, del_book_id):
        try:
            # BOOK_ID is INT — no quotes needed
            self.cursor.execute("DELETE FROM BOOKS WHERE BOOK_ID=%s", (del_book_id.get(),))
            self.db.commit()
    ...
    def issue_books(self, std_name, class_, section, roll, issue_bid, issue_date):
        try:
            ...
            date_parts = issue_date.get().split('-')
            if len(date_parts) != 3:
            ...
            # BOOK_ID is INT — no quotes needed
            self.cursor.execute("SELECT QTY FROM BOOKS WHERE BOOK_ID=%s", (issue_bid.get(),))
            ...
            if (len(result) != 0) and (result[0][-1] != 0):
                ...
                try:
                    self.cursor.execute("SELECT BOOK_PRICE FROM BOOKS WHERE BOOK_ID=%s", (issue_bid.get(),))
                ...
                val = (
                    ...
                    class_.get().upper(),
                    section.get().upper(),
                    roll.get(),           # ROLL_NO as string (VARCHAR)
                    issue_bid.get(),
                    int(bprice),
                    mysql_date            # Converted to YYYY-MM-DD for MySQL
                )

                self.cursor.execute(sql, val)
                self.db.commit()

                self.cursor.execute("UPDATE BOOKS SET QTY=QTY-1 WHERE BOOK_ID=%s", (issue_bid.get(),))
                self.db.commit()
    ...
    def return_books(self, return_date, return_stdid):
        try:
            # FIX: Convert DD-MM-YYYY input to date object
            date_parts = return_date.get().split('-')
            if len(date_parts) != 3:
                messagebox.showerror("Error", "Invalid date format! Use DD-MM-YYYY")
                return
            date1 = date(int(date_parts[2]), int(date_parts[1]), int(date_parts[0]))
            ...
            if days > 7:
                self.cursor.execute(
                    "UPDATE ISSUED_BOOKS SET FINE=%s WHERE STUDENT_ID=%s",
                    ((days - 7) * self.fine, return_stdid.get())
                )
                self.db.commit()

            self.cursor.execute("SELECT BOOK_ID FROM ISSUED_BOOKS WHERE STUDENT_ID=%s", (return_stdid.get(),))
            return_bid = self.cursor.fetchall()[-1][0]

            # FIX: Convert DD-MM-YYYY to YYYY-MM-DD for MySQL storage
            mysql_return_date = f"{date_parts[2]}-{date_parts[1]}-{date_parts[0]}"
            self.cursor.execute(
                "UPDATE ISSUED_BOOKS SET RETURN_DATE=%s WHERE STUDENT_ID=%s",
                (mysql_return_date, return_stdid.get())
            )
            self.db.commit()

            # BOOK_ID is INT — no quotes needed
            self.cursor.execute("UPDATE BOOKS SET QTY=QTY+1 WHERE BOOK_ID=%s", (return_bid,))
            self.db.commit()
    ...
    def view_issued_books(self):
        ...
        try:
            ...
            messagebox.showerror("Error","Please try to save the file with different name!")
    ...
    def reset_password(self, new_passwrd, username, old_passwrd):
        ...
        try:
            self.cursor.execute(
                "UPDATE LIB_USERS SET PASSWORD=%s WHERE USERNAME=%s AND PASSWORD=%s",
                (new_passwrd_hash, username, old_passwrd_hash)
            )
            self.db.commit()
        except:
    ...
    def issue_books_gui(self):
        ...
        # FIX: Label updated to DD-MM-YYYY
        ck.CTkLabel(issue_frame, text="Issue Date (DD-MM-YYYY) -", text_font=("Roboto",18)).place(relx=0.05, rely=0.70)
    ...
    def return_books_gui(self):
        ...
        # FIX: Label updated to DD-MM-YYYY
        ck.CTkLabel(return_frame, text="Return Date (DD-MM-YYYY) -", text_font=("Roboto",20)).place(relx=0.05, rely=0.48)
    ...
    def reset_password_gui(self):
        ...
        reset_btn = ck.CTkButton(self.reset, text="Submit", width=240, text_font=("Roboto",18),
                                 command=lambda: self.reset_password(new_passwrd.get(), username.get(), old_passwrd.get()))
    ...
    def load(self):
        ...
        ck.CTkLabel(load, text="VCET,Viraganoor", text_font=("Courier New",25), text_color="lime").place(relx=0.3, rely=0.05)
        ck.CTkLabel(load, text="Mathesh", text_font=("Courier New",25), text_color="lime").place(relx=0.42, rely=0.11)
    ...
    def login(self):
        ...
        submit = ck.CTkButton(login_screen, text="Submit", text_font=("Roboto",12),
                              command=lambda: self.check_creds(login_screen, username, passwrd))
